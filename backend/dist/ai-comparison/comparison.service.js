...
class ComparisonService {
    async compareProperties(userId) {
      ...
      `, [userId]);
        ...
        const cheapest = properties.reduce((best, property) => Number(property.price) < Number(best.price) ? property : best);
        const largest = properties.reduce((best, property) => Number(property.area) > Number(best.area) ? property : best);
        const mostBedrooms = properties.reduce((best, property) => property.bedrooms > best.bedrooms ? property : best);
        return [
