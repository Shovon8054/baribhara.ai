...
const ensureAdminUser = async () => {
    try {
        ...
        if (checkRes.rows.length === 0) {
            await pool.query(`INSERT INTO users (email, password, full_name, role, is_verified, is_active)
         VALUES ($1, $2, 'BashaBhara Admin', 'ADMIN', true, true)`, [email, hashedPassword]);
            console.log("Admin user admin@baribhara.ai created successfully.");
        }
        else {
            await pool.query(`UPDATE users SET password = $1, role = 'ADMIN', is_verified = true, is_active = true WHERE email = $2`, [hashedPassword, email]);
