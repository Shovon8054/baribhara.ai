...
class AIService {
    async listingAssistant(action, data, text) {
...
PENTHOUSE, or DUPLEX. Treat phrases like "duplex flat" as DUPLEX.`,
            ...
            return text ? this.normalizeFilters(JSON.parse(text)) : {};
        }
        catch (error) {
    ...
    extractFiltersLocally(userQuery) {
        ...
        if (budget !== undefined && maxPrice === undefined)
        ...
        if (minArea)
            filters.minArea = Number(minArea.replace(/,/g, ""));
        if (maxArea)
            filters.maxArea = Number(maxArea.replace(/,/g, ""));
        ...
        if (query.includes("family"))
            filters.family_bachelor = "FAMILY";
        if (query.includes("bachelor"))
            filters.family_bachelor = "BACHELOR";
        if (query.includes("unfurnished"))
            filters.furnished = "false";
        else if (query.includes("furnished"))
            filters.furnished = "true";
        if (query.includes("without parking") || query.includes("no parking"))
            filters.parking = "false";
        else if (query.includes("parking"))
            filters.parking = "true";
        if (query.includes("without lift") || query.includes("no lift"))
            filters.lift = "false";
        else if (query.includes("lift"))
            filters.lift = "true";
        if (query.includes("not pet friendly") || query.includes("no pets"))
            filters.pet_friendly = "false";
        else if (query.includes("pet friendly") || query.includes("pets allowed"))
        ...
        else if (query.includes("smallest"))
        ...
        if (propertyType)
            filters.property_type = propertyType.toUpperCase();
    ...
    normalizeFilters(filters) {
        ...
        if (typeof normalized.property_type === "string") {
            ...
            if (value.includes("DUPLEX"))
                normalized.property_type = "DUPLEX";
            else if (value.includes("PENTHOUSE"))
                normalized.property_type = "PENTHOUSE";
            else if (value.includes("APARTMENT"))
                normalized.property_type = "APARTMENT";
            else if (value.includes("STUDIO"))
                normalized.property_type = "STUDIO";
            else if (value.includes("HOUSE"))
                normalized.property_type = "HOUSE";
            else if (value.includes("FLAT"))
                normalized.property_type = "FLAT";
            else
        ...
        if (typeof normalized.family_bachelor === "string") {
            ...
            if (["FAMILY", "BACHELOR", "ANY"].includes(value))
                normalized.family_bachelor = value;
            else
                delete normalized.family_bachelor;
        }
        for (const key of ["furnished", "parking", "lift", "pet_friendly"]) {
            if (typeof normalized[key] === "boolean")
                normalized[key] = String(normalized[key]);
        }
        return normalized;
    }
    listingFallback(action, data, text) {
        if (action === "title") {
